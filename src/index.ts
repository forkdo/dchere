...
// Docker镜像仓库主机地址
let hub_host: string = 'registry-1.docker.io';
// Docker认证服务器地址
const auth_url: string = 'https://auth.docker.io';

let 屏蔽爬虫UA: string[] = ['netcraft'];
...
export default {
  async fetch(request: Request, env: any, ctx: ExecutionContext): Promise<Response> {
    ...
    // 获取请求参数中的 ns
    const ns: string | null = url.searchParams.get('ns');
    const hostname: string = url.searchParams.get('hubhost') || url.hostname;
    const hostTop: string = hostname.split('.')[0];
    ...
    const fakePage: boolean = checkHost ? checkHost[1] : false;
    console.log(`域名头部: ${hostTop} 反代地址: ${hub_host} searchInterface: ${fakePage}`);
    // 更改请求的主机名
    url.hostname = hub_host;
    const hubParams: string[] = ['/v1/search', '/v1/repositories'];
    if (屏蔽爬虫UA.some(fxxk => userAgent.includes(fxxk)) && 屏蔽爬虫UA.length > 0) {
      ...
      if (url.pathname === '/') {
        ...
        if (qParam && qParam.includes('library/') && qParam !== 'library/') {
          url.searchParams.set('q', qParam.replace('library/', ''));
        }
    ...
    if (url.pathname.includes('/token')) {
      ...
      const token_url: string = auth_url + url.pathname + url.search;
    ...
    const wwwAuth = new_response_headers.get("Www-Authenticate");
    if (wwwAuth) {
      const re = new RegExp(auth_url, 'g');
      new_response_headers.set("Www-Authenticate", wwwAuth.replace(re, workers_url));
    }
    ...
    if (location) {
      ...
      return await httpHandler(request, location, hub_host);
    }
